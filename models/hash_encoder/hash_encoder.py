...
class HashEncoderFunc(Function):
    ...
    def forward(ctx,x,embeddings,args):
        ...
        y=torch.empty(args['B'],args['L']*args['C'],device=x.device, dtype=args['dtype']) # y: B,L,C
        dy_dx=torch.empty(args['B']*args['L']*args['D']*args['C'],device=x.device, dtype=args['dtype']) # dy/dx: B,L,D,C, if_cal_grad_x is True 计算grad_x, 需提前计算dy/dx。
        backend.hash_encoder_forward(x,embeddings,args['offsets'],args['resolution_list'],args['B'],args['D'],args['C'],args['L'],args['if_cal_grad_x'],y,dy_dx)
...
class HashEncoderBackwardFunc(Function):
    ...
    def forward(ctx, grad_y, x, embeddings,dy_dx,args):
        ...
        backend.hash_encoder_backward(grad_y,x,dy_dx,args['offsets'],args['resolution_list'],args['B'],args['D'],args['C'],args['L'],args['if_cal_grad_x'],grad_x,grad_embeddings)
    ...
    def backward(ctx, grad2_grad_x, grad2_grad_embeddings):
        ...
        # TODO: 实现grad2_x, 配合torch.autograd_grad可实现二阶曲率hessian的计算。二阶要求非线性interpolation。[√] （3-7 01：55， 重大疑问，我if_cal_hessian_x设置为false还是能计算hessian，额？？？？我不是白写了吗什么意思？）
        grad2_x = torch.zeros_like(x)
        backend.hash_encoder_second_backward(grad2_grad_x,x,embeddings,grad_y,dy_dx,args['offsets'],args['resolution_list'],args['B'],args['D'],args['C'],args['L'],args['if_cal_hessian_x'],grad2_embeddings, grad2_grad_y, grad2_x)
...
class HashEncoder(nn.Module):
    ...
    def forward(self, x, if_cal_hessian_x=False):
        ...
        args = {'offsets':self.offsets,
                ...
                'if_cal_hessian_x': if_cal_hessian_x,
                'dtype':self.dtype}
...
'''
# hash_encoder=HashEncoder(x_dim=2,resolution_list=[2,4,8],type='half').cuda()
# print('embeddings',hash_encoder.embeddings)
# x=torch.tensor([[0,0]],dtype=torch.float32,requires_grad=True).cuda()
# y=hash_encoder(x)
# print('y',y)
# grad_x=torch.autograd.grad(y.sum(),x,retain_graph=True,create_graph=True)[0]
# print('grad_x',grad_x)
