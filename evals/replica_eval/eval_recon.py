...
def calc_3d_metric(rec_meshfile, gt_meshfile, align=False):
    ...
    # normal consistency
    # surface上随机采样点，计算法向量-cosine相似度
    N = 200000
    pointcloud_pred, idx = mesh_rec.sample(N, return_index=True) # mesh.sample: 从surface随机采样点
    ...
    _, index1 = nn_correspondance(pointcloud_pred, pointcloud_trgt) # gt->pred的近邻点: completion
    _, index2 = nn_correspondance(pointcloud_trgt, pointcloud_pred) # pred->gt的近邻点: accuracy
    ...
    chamfer_dist = (accuracy_rec + completion_rec) * 0.5
...
if __name__ == '__main__':
    ...
    parser.add_argument('--rec_mesh', type=str,default='evaluation/replica_grids/office2.ply',
                        help='reconstructed mesh file path')
    parser.add_argument('--gt_mesh', type=str,default='../data/Replica/cull_GTmesh/office2.ply',
