...
class MonoSDFDataset(Dataset):
    def __init__(self, data_dir, scan_id=-1, split='train', num_rays=1024) -> None:
        super().__init__()
        self.data_dir=data_dir if scan_id == -1 else os.path.join(data_dir, f'scan{scan_id}')
        # self.data_dir = data_dir if scan_id == -1 else os.path.join(data_dir, f'scene{scan_id}')
        self.split=split
        ...
        self.has_mono_depth = data["has_mono_prior"]
        self.has_mono_normal = data["has_mono_prior"]
        # self.has_mono_depth = True
        # self.has_mono_normal = True
        self.has_mask = False # TODO: 添加2d mask, guide osf
        self.has_uncertainty = False # TODO: 添加2d uncertainty
    ...
    def load_data(self, idx):
        ...
        if self.has_mono_normal:
            ...
            normal = normal.reshape(3, -1).float() # monosdf的normal是(3, h*w)
            normal = normal * 2 - 1
            normal = normal.T
        else:
        ...
        if self.has_uncertainty:
            uncertainty = torch.from_numpy(np.load(self.uncertainty_paths[idx])).float().reshape(-1, 1)
        else:
            uncertainty = torch.zeros_like(depth)
        return rgb, depth, normal, mask, uncertainty
    ...
    def __getitem__(self, idx):
        ...
        rgb, depth, normal, mask, uncertainty= self.load_data(idx)
        x, y = np.meshgrid(np.arange(0, self.w, 1), np.arange(0, self.h, 1))
        if self.split == 'valid':
