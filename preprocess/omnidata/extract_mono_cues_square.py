...
def get_args():
    ...
    args.add_argument('--input_dir', type=str, default=None, help="输入图像分辨率宽高比等于1为正方形时...")
...
def main(args): 
    ...
    image_size = 384
    pretrained_weights_path = os.path.join('./omnidata/pretrained_models', f"omnidata_dpt_{args.task}_v2.ckpt")  # 'omnidata_dpt_depth_v1.ckpt'
    ...
    img_paths = glob.glob(os.path.join(args.input_dir, "**", '*.jpg'), recursive=True)
    img_paths.sort()
